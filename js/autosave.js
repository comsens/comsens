            ...
            $.get(g5_bbs_url+"/ajax.autosavelist.php", function(data){
                ...
                if ($(data).find("list").find("item").length > 0) {
                    $(data).find("list").find("item").each(function(i) {
                        ...
                        $("#autosave_pop ul")
                            .append('<li class="list-group-item"><a href="#none" class="autosave_load">'+subject+'</a><span class="pull-right">'+datetime+' <button type="button" class="btn btn-danger btn-xs  autosave_del">삭제</button></span></li>')
                            .find("li:eq("+i+")")
